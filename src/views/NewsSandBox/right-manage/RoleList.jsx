...
export default function RoleList() {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (item) => <Space size="middle">
        ...
        <Button icon={<EditTwoTone />} size="large" shape="circle" onClick={() => showModal(item)}/>
  ...
  const handleOk = () => {
    ...
    axios.patch(`http://localhost:3006/roles/${roleId}`, {rights: defaultCheckedKeys})
  ...
  const onCheck = (value) => {
    console.log(value)
    setDefaultCheckedKeys(value)
  }
  return (
    <div>
      <Table dataSource={dataSource} columns={columns} rowKey={(item) => item.id}></Table>
      <Modal title="权限分配" visible={isModalVisible} onOk={handleOk} onCancel={closeModal}>
      <Tree
      checkable
      treeData={rightList}
